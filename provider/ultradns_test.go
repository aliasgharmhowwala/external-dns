...
func TestUltraDNSProvider_ApplyChangesCNAME(t *testing.T) {
	...
	err := provider.ApplyChanges(context.Background(), changes)
	assert.Equal(t,err,"The CNAME and TXT Record name cannot be same please recreate external-dns with - --txt-prefix=")
}
...
func TestNewUltraDNSProvider_FailCases(t *testing.T) {
	...
	_, err := NewUltraDNSProvider(endpoint.NewDomainFilter([]string{"test-ultradns-provider.com"}), true)
	assert.Nilf(t,err,"Pool Type other than given type not working %s","formatted")
	...
	_, err = NewUltraDNSProvider(endpoint.NewDomainFilter([]string{"test-ultradns-provider.com"}), true)
	assert.Nilf(t,err,"Probe value other than given values not working  %s","formatted")	
	...
	_, err = NewUltraDNSProvider(endpoint.NewDomainFilter([]string{"test-ultradns-provider.com"}), true)
	assert.Nilf(t,err,"ActOnProbe value other than given values not working %s","formatted")	
	...
	_ = os.Setenv("ULTRADNS_ACCOUNTNAME", "")
	_, err = NewUltraDNSProvider(endpoint.NewDomainFilter([]string{"test-ultradns-provider.com"}), true)
	assert.Nilf(t,err,"Expected to give error if password is not set %s","formatted")
